...
<strong>Code Splitting</strong> split code into smaller chunks to improve initial load times and reduce bundle size.<br />
<strong>Linting and Formatting</strong> to code consistency and quality using linters like ESLint and code formatters. Follow the coding standards and best practices to ensure code maintainability and readability.<br />
<strong>Documentation</strong> for code components, APIs and architecture to understanding among team members. Use README file for project-level documentation.
          </p>)
        }
...
<strong>Functional Components</strong> are JavaScript functions that return JSX. They are simpler, easier to read, and typically used with React Hooks to handle state and lifecycle methods.<br />
...
React doesn't support <strong>Error Boundaries</strong> with hooks, but you can still handle logic errors<br />
<strong>Form Validation</strong> error handling with form libraries or custom logic<br />
<strong>Handling Errors in API calls</strong> are a common source of errors. We can manage these errors by showing a loading state and an error message.
          </p>)
        }
...
<strong>Redux</strong> store and manage application state. It provides APIs to change and access the application's state. It is helpfull to sharing data from one component to another component. Store is used to save the entire State at one place. Action are plain JavaScript object that contains information.<br />
<strong>Context API</strong> is used to share data between components. It's useful for sharing global data that many components need to access. Context API creates a global state that can be accessed by any component. Context API consists of two main components: Provider and Consumer. Provider component provides data to the context. Consumer component consumes data from the context.<br />
<strong>State</strong> is an object which stores the property values of component. State helpful in respond to user input, API responses and other events. State is used for managing and persisting data within a component. When the state object changes the component re-renders. To update the state we can use the setState function. We can add a state variable by calling the useState Hook. The useState Hook returns a pair of values: the current state and the function to update it<br />
<strong>Props</strong> are objects that pass data from one component to another. Props are read-only means the child component cannot change the data from the parent component. Props can be used to pass any JavaScript value including objects, arrays, and functions. Props can be used to make components more dynamic.
</p>)
        }
